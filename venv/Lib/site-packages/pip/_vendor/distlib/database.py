...
class DistributionPath(object):
    ...
    def _yield_distributions(self):
        ...
        for path in self.path:
            ...
            if finder is None:
                continue
            r = finder.find_element('')
            if not r or not r.is_container:
                continue
            rset = sorted(r.resources)
            for entry in rset:
                r = finder.find_element(entry)
                ...
                try:
                    if self._include_dist and entry.endswith(DISTINFO_EXT):
                        ...
                        for metadata_filename in possible_filenames:
                            ...
                            pydist = finder.find_element(metadata_path)
                            if pydist:
                                break
                        else:
                            continue
...
class InstalledDistribution(BaseInstalledDistribution):
    ...
    def __init__(self, path, metadata=None, env=None):
        ...
        if env and env._cache_enabled and path in env._cache.path:
            ...
            r = finder.find_element(METADATA_FILENAME)
            # Temporary - for Wheel 0.23 support
            if r is None:
                r = finder.find_element(WHEEL_METADATA_FILENAME)
            # Temporary - for legacy support
            if r is None:
                r = finder.find_element(LEGACY_METADATA_FILENAME)
            if r is None:
        ...
        r = finder.find_element('REQUESTED')
    ...
    def get_distinfo_resource(self, path):
        ...
        return finder.find_element(path)
